...
def adc(array, B_ADC, MAC_length):
    return (np.round((array/MAC_length)*(2**B_ADC - 1)))*(MAC_length/(2**B_ADC - 1))

def conv2d(input, filters, MAC_length, B_ADC, bias=None, stride=1, padding=0):
    N, C, H, W = input.shape
    ...
    iter = col_bin.shape[1] % MAC_length + 1
    in_X, in_Y = col_bin.shape
    filt_X, filt_Y = filters_bin.shape
    col_accum = np.zeros((in_X, filt_Y))

    for i in range(iter):
        if i != (iter-1):
            prod = np.dot(col_bin[:, i*MAC_length : (i+1)*MAC_length], filters_bin[i*MAC_length : (i+1)*MAC_length, :])
            col_accum += adc(prod, B_ADC, MAC_length=MAC_length)
        else:
            prod = np.dot(col_bin[:, i*MAC_length:], filters_bin[i*MAC_length: , :])
            col_accum += adc(prod, B_ADC, MAC_length=MAC_length)
...
def main():
    ...
    output, conv_dims = conv2d(input_binary, weight_binary, MAC_length=args.MAC_length, B_ADC=args.B_ADC, bias=None, stride=2, padding=3)
    ...
    torch.save(conv_out_tens, f'{args.B_ADC}BADC_{args.MAC_length}MAClength.pt')
    # print(conv_out_tens)
