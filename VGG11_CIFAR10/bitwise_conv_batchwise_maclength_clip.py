...
def clip_adc(array, B_ADC, clip):
    return (torch.round((torch.clamp(array, min=0, max=clip)/clip)*(2**B_ADC - 1)))*(clip/(2**B_ADC - 1))

def conv2d_5d(input, filters, B_ADC, MAC_length, clip, bias=None, stride=1, padding=0):
    ...
    for i in range(iter):
        if i != (iter - 1):
            ...
            col_accum += clip_adc(prod, B_ADC, clip=clip)
        else:
            prod = torch.matmul(col_bits[:, i * MAC_length:].float(), filters_bits[i * MAC_length:, :].float())
            # col_accum += adc(prod, B_ADC, MAC_length=MAC_length)
            col_accum += clip_adc(prod, B_ADC, clip=clip)
        
        # torch.save(prod, f'MACVALUES_ITER{iter}.pt')
...
def main():
    ...
    parser.add_argument('--clip', type=int, required=True, help='OCC Clip Point')
    ...
    for i in range(0, N, batch_size):
        ...
        output, out_dims = conv2d_5d(inp_batch, weight_5d_bin, B_ADC=args.B_ADC, MAC_length=args.MAC_length, clip=args.clip, stride=args.stride, padding=args.padding)
    ...
    output_file = os.path.join(output_subdir, f'CONV_OUT_LAYER{args.layer}_{args.x_bits}BX_{args.w_bits}BW_{args.B_ADC}BADC_{args.MAC_length}MAC_{args.clip}CLIP.pt')
