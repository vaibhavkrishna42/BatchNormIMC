...
def conv2d_5d(input, filters, bias=None, stride=1, padding=0):
    ...
    output = torch.matmul(col_bits.float(), filters_bits.float())
    
    if bias is not None:
        output += bias
    
    return output, (out_dims[0], N_F, out_dims[1], out_dims[2])
...
def main():
    ...
    conv_out_2d, conv_out_dims = conv2d_5d(inp_5d_bin, weight_5d_bin, stride=args.stride, padding=args.padding)
    conv_out_4d = twos_power_mult(conv_out_2d, x_bits, w_bits, conv_out_dims)

    conv_out_final = conv_out_4d / (scale_x * scale_w).item()
    print(f"Convolution output dims: {conv_out_final.shape}")

    output_subdir = f'conv_out/conv{args.layer}'

    os.makedirs(output_subdir, exist_ok=True)
    output_file = os.path.join(output_subdir, f'CONV_OUT_LAYER{args.layer}_{args.x_bits}BX_{args.w_bits}BW.pt')
