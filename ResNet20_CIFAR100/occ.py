...
if __name__ == "__main__":
    ...
    for layer_num in range(1, args.layers + 1):
        file_name = fr"OCC_MACValues\conv{layer_num}\MACVALUES_LEN256_LAYER{layer_num}_ITER0.pt"

        iter1_torch = torch.load(file_name, weights_only=True).to('cpu')
        iter1 = iter1_torch.numpy()
        # Print if you want to see the number of MAC values that are being used to find the clip point
        # print(f"{iter1.reshape(-1).shape[0]} MAC Values")
        results_occ = clip(iter1.reshape(-1), args.xr_start, args.xr_end, args.B_start, args.B_end, layer=layer_num)

        # print("B", "Clip", "MSE", f"for Layer {layer_num}")
        for i in results_occ:
            # print(i[0], f"{i[1]}, {round(i[2].item(), 2)}")
            print(f"{i[1]} {round(i[2].item(), 2)}", end=" ")
        print("")
